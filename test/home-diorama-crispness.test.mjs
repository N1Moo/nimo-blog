...
test("home canvases keep the original crisp DPR caps", () => {
...
test("home story overlay composites cached desktop frames at native pixel scale", () => {
  assert.match(
    dioramaTs,
  ...
  assert.doesNotMatch(dioramaTs, /STORY_FRAME_RENDER_MAX_DIMENSION/);
...
test("home story overlay keeps crisp single keyframes while texture redraws stay stepped", () => {
  assert.match(dioramaTs, /const frameIndex = Math\.round\(clamp\(storyInput\.progress\) \* STORY_FRAME_STEPS\);/);
  assert.doesNotMatch(dioramaTs, /frameBlend|globalAlpha = opacity/);
  assert.match(
    dioramaTs,
    /Math\.abs\(storyProgress - lastTexturedStoryProgress\) >= SCREEN_REDRAW_STEP/,
  );
  assert.match(dioramaTs, /if \(needScreenRedraw \|\| progressChanged\)/);
});
...
test("home renderer caches static shadows and only recompiles opacity materials on mode changes", () => {
  assert.match(dioramaTs, /renderer\.shadowMap\.autoUpdate = false;/);
  assert.match(dioramaTs, /renderer\.shadowMap\.needsUpdate = true;/);
  assert.match(
    dioramaTs,
    /if \(mat\.transparent !== transparent\) \{\s*mat\.transparent = transparent;\s*mat\.needsUpdate = true;\s*\}\s*mat\.opacity = opacity;/,
  );
  assert.doesNotMatch(
    dioramaTs,
    /mat\.opacity = opacity;\s*mat\.needsUpdate = true;/,
  );
  assert.match(dioramaTs, /const STORY_FRAME_STEPS = useMobileCarrier \? 120 : 140;/);
  assert.match(dioramaTs, /const STORY_CONNECTOR_MOTION_FPS = 18;/);
  assert.match(dioramaTs, /const HOME_PROGRESS_FOLLOW_RATE = 0\.018;/);
  assert.match(
    dioramaTs,
    /homeProgress = Math\.abs\(nextProgress - homeProgress\) < 0\.00001\s*\? nextProgress\s*:\s*lerp\(homeProgress, nextProgress, progressFollow\);/,
  );
});
