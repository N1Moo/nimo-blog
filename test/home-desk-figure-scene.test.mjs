...
test("home first-screen diorama keeps the configured title as the centered hero while the model stays subordinate", () => {
...
test("home 3D desk figure can replace the fallback figure with the Sketchfab typing character", () => {
  ...
  assert.match(dioramaTs, /typingCharacterMount\.visible = true;/);
  assert.match(dioramaTs, /let typingCharacterMixer: THREE\.AnimationMixer \| null = null;/);
  ...
  assert.match(
    dioramaTs,
    /if \(typingCharacterMixer && !reduceMotion\) \{\s*typingCharacterMixer\.update\(dt \/ 1000\);\s*applyTypingCharacterCorrectivePose\(\);\s*\}/,
  );
  assert.match(dioramaTs, /typingCharacterMixer\?\.stopAllAction\(\);/);
...
test("home first-screen 3D component stays small enough to support the configured hero", () => {
...
test("home startup keeps the real opening frame while heavy assets warm up", () => {
  ...
  assert.match(dioramaTs, /let animationLoopActive = false;/);
  assert.match(dioramaTs, /const renderBootFrame = \(\) => \{/);
  assert.match(dioramaTs, /const startAnimationLoop = \(\) => \{/);
  ...
  assert.match(dioramaTs, /renderBootFrame\(\);\s*startAnimationLoop\(\);/);
  assert.doesNotMatch(dioramaTs, /const resize = \(\) => \{[\s\S]*renderBootFrame\(\);[\s\S]*\};\s*resize\(\);/);
  assert.match(dioramaTs, /keyboardModelLoaded = true;\s*syncKeyboardModelOpacity\(mats\.key\.opacity\);\s*renderBootFrame\(\);\s*releaseStartupGate\(\);/);
  assert.match(dioramaTs, /applyTypingCharacterCorrectivePose\(\);\s*\}\s*renderBootFrame\(\);\s*releaseStartupGate\(\);/);
  assert.match(dioramaTs, /const startupGateTimer = window\.setTimeout\(\(\) => \{\s*startupGateForcedOpen = true;\s*releaseStartupGate\(\);\s*\}, STARTUP_GATE_TIMEOUT_MS\);/);
