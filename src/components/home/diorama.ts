...
const HOME_PROGRESS_FOLLOW_RATE = 0.018;
...
export function initDiorama() {
  ...
  const syncKeyboardModelOpacity = (opacity: number) => {
    if (Math.abs(lastKeyboardModelOpacity - opacity) < 0.001) return;
    lastKeyboardModelOpacity = opacity;
  ...
  let lastKeyboardModelOpacity = Number.NaN;
  ...
  let typingCharacterMixer: THREE.AnimationMixer | null = null;
  ...
  const syncTypingCharacterOpacity = (opacity: number) => {
    if (Math.abs(lastTypingCharacterOpacity - opacity) < 0.001) return;
    lastTypingCharacterOpacity = opacity;
  ...
  let lastTypingCharacterOpacity = Number.NaN;
  ...
  gltfLoader.load(
    ...
    (gltf) => {
      ...
      if (gltf.animations.length) {
        typingCharacterMixer = new THREE.AnimationMixer(model);
  ...
  const sceneScreenColor = new THREE.Color();
  const sceneRoomColor = new THREE.Color();
  ...
  const animate = () => {
    ...
    if (typingCharacterMixer && !reduceMotion) {
      typingCharacterMixer.update(dt / 1000);
      applyTypingCharacterCorrectivePose();
    }
    ...
    const progressFollow = reduceMotion
      ? 1
      : 1 - Math.exp(-dt * HOME_PROGRESS_FOLLOW_RATE);
    homeProgress = Math.abs(nextProgress - homeProgress) < 0.00001
      ? nextProgress
      : lerp(homeProgress, nextProgress, progressFollow);
    renderMode = getRenderMode(homeProgress);
    visualProgress = getStoryVisualProgress(homeProgress);
    const storyProgress = getRenderedStoryProgress();
    ...
    if (renderMode === "story") {
      ...
      if (needScreenRedraw || progressChanged) {
        const updateTexture = needScreenRedraw && shouldUpdateScreenTexture(homeProgress);
        needScreenRedraw = false;
        drawScreen({ overlay: true, texture: updateTexture }, now);
      }
    ...
    scene.background = sceneBackgroundColor
      .copy(sceneScreenColor.setHex(THEMES[theme].screenBg))
      .lerp(sceneRoomColor.setHex(THEMES[theme].sceneBg), outsideReveal);
  ...
  drawScreen();
  const startupGateTimer = window.setTimeout(() => {
  ...
  const cleanup = () => {
    ...
    typingCharacterMixer?.stopAllAction();
    typingCharacterMixer = null;
